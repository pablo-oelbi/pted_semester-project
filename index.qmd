...
The purpose of our analysis was to statistically investigate differences in three measures (distance, total_time, and direct_distance) across five modes of transport (Walk, Car, Bike, Bus, Tram). In order to choose an appropriate test for comparing these groups, we first checked the assumptions of normality and homogeneity of variances.

The Shapiro-Wilk test was used to test for normality in each measure across the different modes of transport. The results showed significant deviation from normality (p \< 0.05) for all combinations of measures and transport modes. This suggests that the data for these groups do not follow a normal distribution.

We also examined the homogeneity of variances using Levene's test, which is crucial for certain statistical methods like the traditional ANOVA. The test results are summarized in the table below:
...
The results of the Levene's test show significant deviation from the assumption of equal variances for 'distance' and 'direct_distance' (p \< 0.05), while 'total_time' did not significantly deviate from this assumption.
...
These results indicate that for distance, Bike, Bus, Car, and Tram all had significantly more distance than Walk. The negative estimate values represent the average difference in distance compared to Walk, with all modes of transport having more distance on average.
...
The probability of choosing Walk decreased significantly as the distance to the train station increased. At shorter distances (e.g., 24 meters), the probability of selecting Walk was high, with approximately 92.7% likelihood and stays over 50% till 1150 meters where it is dropping below. That means that every second individual is chosing a different transport mode then walking when the train station is away more then 1150 meters. On 1700 meters just every fifth individual is chosing to go by walk. Starting from 2100 meters the probability drops to less then 10%.

Overall, as the distance increased to 2500 meters and longer, the probability diminished to an small value, indicating a strong aversion to walking longer distances.

As the distance increases, the probability of choosing the "Bike" mode also increases. The probabilities start at 1.38% and gradually rise until they reaches it's maximum at 18.55%. This indicates that as the distance to the destination increases, there is a higher likelihood of choosing the "Bike" mode as the preferred transport mode.

Similar patterns were observed for Bus, Car, and Tram. The probabilities of choosing these modes increases as the distance increased. Each mode exhibited distinct probabilities at various distances, signifying the nuanced preferences of individuals.
