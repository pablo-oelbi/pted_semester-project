...
A summary of the enhanced data is shown in @tbl-2.
...
```{r}
#| label: fig-3
#| fig-cap: "Boxplot of direct_distance by transport mode"
...
```{r}
#| label: fig-4
#| fig-cap: "Boxplot of total_time by transport mode"
...
The results of the Levene's test show significant deviation from the assumption of equal variances for 'distance' and 'direct_distance' (p \< 0.05), while 'total_time' did not significantly deviate from this assumption (see @tbl-3).
...
: Results of Levene's test {#tbl-3}
```{r, echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
The results for Welch's ANOVA tests are as follows (@tbl-4):  
...
: Results for Welch's ANOVA tests {#tbl-4}
...
The results of the Games-Howell post-hoc test showed significant differences in pairwise comparisons. Below are the statistically significant results (@tbl-5):
...
: Results for Games-Howell post-hoc test {#tbl-5}
...
kable(tidy_model_summary, digits = 5, caption = "Summary of the regression model of transport mode and distance {#tbl-6}", label = "#tbl-6")
...
```{r}
#| label: fig-5
#| fig-cap: "Visualization of the regression model of transport mode and distance"
# Create the plot
...
The analysis of the distance already revealed substantial insights, the same analysis with the direct distance to the train station and the selection of the transport mode shows even stronger effects and sensitivities (@tbl-7 and @fig-6). All results are comparable, significant as the effects of distance. 
...
# Create a nice-looking table
kable(tidy_model_summary, digits = 5, caption = "Summary of the regression model of transport mode and direct distance {#tbl-7}", label = "#tbl-7")
...
```{r}
#| label: fig-6
#| fig-cap: "Visualization of the regression model of transport mode and direct distance"
# Create the plot
plot <- ggplot(result, aes(x = total_time_value)) +
    geom_line(aes(y = Walk, color = "Walk"), linetype = "dashed") +
    geom_line(aes(y = Bike, color = "Bike")) +
...
# Create a nice-looking table
kable(tidy_model_summary, digits = 5, caption = "Summary of the regression model of transport mode and time {#tbl-8}", label = "#tbl-8")
#plot(Effect("direct_distance",fit),style="stacked")
...
: Probabilities of choosing different transport modes at different time values {#tbl-9}

As the total time value increases, the predicted probabilities for going by walk decreases strongly (@fig-7). 



```{r}
#| label: fig-7
#| fig-cap: "Visualization of the regression model of transport mode and time"
# Create the plot
plot <- ggplot(result, aes(x = total_time_value)) +
    geom_line(aes(y = Walk, color = "Walk"), linetype = "dashed") +
    geom_line(aes(y = Bike, color = "Bike")) +
